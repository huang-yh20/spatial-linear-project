...
activation_func_list = [activation_func_dict[p_simul_tanhlinear.activation_func[0]], activation_func_dict[p_simul_tanhlinear.activation_func[1]]]
...
for trial_plot in trange(len(file_name_list)):
    ...
    #plot dyn of neurons
    record_x = np.load(r"./data/artfigs_NC_"+'d_II_g_bar_II'+'_'+str(trial_params[0])+'_'+str(trial_params[1])+'_'+ str(0)+r'.npy')
    ...
    for trial_show in range(t_show_num):
        ...
        cb.update_ticks()
        plt.tight_layout()
        plt.savefig(r"./figs/artfigs_NC_variousdyn_dynimag_"+str(trial_plot)+"_"+str(trial_show)+".png")
        plt.close()
